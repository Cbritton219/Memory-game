document.addEventListener("DOMContentLoaded", function() {
    ...
    function startGame() {
      ...
      for (let i = 0; i < pairs.length; i++) {
        let path = "image/" + pairs[i] + ".jpg";
        console.log(path)
        cards[i].children[1].children[0].src = path;
      }


      let game_cards = document.getElementsByClassName("game-card");
      console.log(game_cards)

      for (iterator in game_cards) {
        let game_card = game_cards[iterator];
        console.log(game_card)
        game_card.onclick = (e) => {
          if (game_card.children[1].style.display === "none") {
            game_card.children[0].style.display = "none";
            game_card.children[1].style.display = "block";
          } else {
            game_card.children[0].style.display = "block";
            game_card.children[1].style.display = "none";
          }
        }
        // game_cards[game_card].addEventListener('click', function handleClick(event) {
    ...
    function setScore(newScore) {
      ...
      document.getElementById("current-start").innerText = currentScore;
    }
